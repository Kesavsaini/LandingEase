...
const Card = ({card}) => {
    const {
      ...
      return (
        <div className={`card bg-base-100  w-full sm:w-72 h-auto shadow-xl ${isBottom && "flex-col-reverse"} ${!isImage && "h-[14rem]"} ${!isButton && "h-[24rem]"}  ${!isButton && !isImage && "h-[10rem]"}`}>
        {isImage && <figure className={`w-full h-[18rem] ${isBottom && "rounded-t-none"}`}>
         <Image src={imgSrc} objectFit='cover' alt="img" width={300} height={300} className={`${isBottom && "rounded-b-lg"} ${isBottom && "rounded-t-none"}`} style={{height:`${imgSize}%`,width:`${imgSize}%`}} />
       </figure>
       }
